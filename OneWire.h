...
// OneWire Interface
//
// Class provide low level communication on a physical interface. The class handles
// interface configuration, reset routine and bit bang communication.
//---------------------------------------------------------------------------------
//
class OwInterface{
    private:
    ...
    public:
        ...
        OwInterface(volatile uint8_t*, volatile uint8_t*,   // Constructor
...
// OneWire Bus
// 
// The class handles the byte communication on a specific interface. It provide
// byte send/receive function on one wire bus.
//
// ToDo:
// - count of present devices on bus
// - managing present devices eg. ROM ID Array and access by index number
//---------------------------------------------------------------------------------
// 
class OwBus{
    private:
...
// OneWire Device
// 
// The Class handles the communication to a specific one wire device. The parent
// class provide standard functions usable for any device on bus. The child classes
// provide special functions for a specific device like DS18B20.
//
// ToDo:
// -move search function into class "interface"
//---------------------------------------------------------------------------------
// 
class OwDevice{
    public:
